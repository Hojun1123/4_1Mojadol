...
from ultralytics.yolo.engine.detect_dumping import main_process
STREAM_WARNING = """
...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        if self.args.conf is None:
            self.args.conf = 0.65  # default conf=0.25  #보여질 인식률 없다면 0.25로 설정
    ...
    def stream_inference(self, source=None, model=None):
        ...
        for batch in self.dataset:
            ...
            # print(self.results)
            ### results[0] > 인식된 객체 개수
            ### if results[0] > 0 이면 1개이상의 객체가 인식됨 이때 각 객체의 정보를 boxes, mask, names ... 등으로 가져올 것.
# #### add custom code
            '''
        orig_img (numpy.ndarray): numpy 배열로 된 원본 이미지입니다.
        orig_shape (튜플): (높이, 너비) 형식의 원본 이미지 모양입니다.
        boxes (Boxes, optional): 감지 경계 상자를 포함하는 Boxes 객체.
        masks (Masks, optional): 감지 마스크를 포함하는 Masks 개체입니다.
        probs (numpy.ndarray, optional): 각 클래스에 대한 탐지 확률의 2D numpy 배열입니다.
        names (List[str]): 클래스 이름 목록.
        path(str): 이미지 파일의 경로입니다.
        _keys(튜플): 비어 있지 않은 속성에 대한 속성 이름의 튜플입니다.
            '''
        # print(len(self.results[0]))
            if len(self.results[0]) > 0:
                ###print("오브젝트 xywh")
                ### . xywh는 중심점 wh, xyxy는 좌qq상단 우하단 포인트 입니다.
                main_process(self.results[0], self.plotted_img)
#### end custom code
